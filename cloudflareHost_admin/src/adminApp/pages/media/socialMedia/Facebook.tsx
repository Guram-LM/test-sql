...
const Facebook:React.FC<SitialProps> = ({data}) => {

  const deleteResource = useDeleteResource("social_media");
  const [confirmDeleteId, setConfirmDeleteId] = useState<number | null>(null);

  const facebook =
    data?.filter(item => item.platform === "facebook").map(item => ({
      id: item.id,
      title: item.title_ka,
      link: item.link,
    ...
    if (facebook.length === 0) return <EmptyState message="სოციალური მედია ჯერ არ არის" />
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute top-0 left-0 w-full h-[420px] bg-[#1877F2] -skew-y-6 origin-top-left"></div>

        <div className="absolute right-[-120px] top-20 text-[420px] font-black text-[#1877F2]/10 select-none pointer-events-none">
          f
        </div>

        <div className="absolute -top-40 -left-40 w-[420px] h-[420px] rounded-full bg-[#1877F2]/20 blur-3xl"></div>
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          <div className="mb-20 text-white">
            ...
            <p className="text-blue-200/80">
              ჩვენი უახლესი პოსტები სოციალური ქსელიდან
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            {facebook.map(post => (
              <article
                key={post.id}
                className="group relative bg-white/5 backdrop-blur-xl border border-blue-500/20 rounded-2xl overflow-hidden shadow-xl transition-all duration-500 hover:shadow-2xl hover:shadow-blue-600/30 hover:-translate-y-2 hover:border-blue-400/40"
              >
                <div className="flex items-center px-4 pt-4 pb-3">
                  <div className="relative flex-shrink-0">
                    <div className="w-10 h-10 rounded-full bg-[#0866FF] flex items-center justify-center shadow-md ring-2 ring-blue-400/30 ring-offset-2 ring-offset-gray-950">
                  ...
                  <div className="ml-3">
                    ...
                    <p className="text-xs text-blue-300/70">1 სთ წინ • 🌐</p>
                  </div>
                </div>
                ...
                {post.image ? (
                  <div className="relative overflow-hidden aspect-[4/3] md:aspect-[5/4]">
                    ...
                    <img
                      src={post.image}
                      alt="Facebook post"
                      className="w-full h-full object-cover transform scale-105 group-hover:scale-110 transition-transform duration-700 ease-out brightness-90 group-hover:brightness-110"
                      loading="lazy"
                    />
                  ...
                  <div className="aspect-[4/3] bg-gradient-to-br from-blue-900/50 to-indigo-900/40 flex items-center justify-center">
                    <span className="text-blue-300 text-4xl opacity-40">🔵</span>
                  </div>
                )}

                <div className="px-4 py-3 flex items-center justify-between text-xs border-t border-blue-500/10 bg-blue-950/20">
                  <span className="text-blue-200">👍 148 • ❤️ 62</span>
                  <button
                    onClick={(e) => {
                      e.preventDefault();      
                      e.stopPropagation();
