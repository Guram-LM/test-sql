...
import { useFetchMyEvents } from "../../../services/queries/events/useFetchEvents";
import { useDeleteMyEvent } from "../../../services/queries/events/useDeleteEvent";
import { useState } from "react";
import EventFormPage from "../events-form/Eventformpage";
import BackButton from "../../../component/back-button/BackButton";

const MyEventsList = () => {
  const { data: events, isLoading } = useFetchMyEvents();
  const deleteEvent = useDeleteMyEvent();
    ...
    <div className="min-h-screen px-10 py-28 2xl:py-10 2xl:pb-22">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 flex items-center gap-3">
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-lg shadow-violet-200 flex-shrink-0">
            <span className="text-xl sm:text-2xl">🗓</span>
          </div>
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">ჩემი ივენთები</h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5">My Events</p>
          </div>
          <span className="ml-auto flex-shrink-0 bg-violet-100 text-violet-700 text-xs sm:text-sm font-semibold px-2.5 sm:px-3 py-1 rounded-full">
            {events?.length || 0} სულ
          </span>
        </div>
        ...
        {!isLoading && (!events || events.length === 0) && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 font-medium">ივენთები არ არის</p>
            <p className="text-gray-300 text-sm mt-1">No events found</p>
          </div>
        )}
            ...
            return (
              <div
                ...
                className={`relative bg-white rounded-2xl border transition-all duration-200 hover:shadow-md overflow-hidden
                  ${past ? "border-gray-100" : "border-violet-100 hover:border-violet-200"}`}
              >

                <div className={`absolute left-0 top-0 bottom-0 w-1
                  ${past ? "bg-gray-200" : "bg-gradient-to-b from-violet-500 to-purple-500"}`}
                />
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={`flex-shrink-0 text-xs font-medium px-2 py-0.5 rounded-full whitespace-nowrap ${
                          past ? "bg-gray-100 text-gray-400" : "bg-violet-50 text-violet-600"
                        }`}>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2.5">
                        ...
                        <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg font-medium ${
                          past ? "bg-gray-50 text-gray-400" : "bg-violet-50 text-violet-700"
                        }`}>
                          🕐 {event.event_time}
                        </span>
                        ...
                        {event.meeting_type === "offline" && event.city && (
                          <span className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg bg-emerald-50 text-emerald-600 font-medium">
                ...
                {event.meeting_link && (
                  <div className={`border-t px-5 sm:px-6 py-2.5 flex items-center justify-between gap-3
                    ${past ? "border-gray-50 bg-gray-50/50" : "border-violet-50 bg-violet-50/40"}`}
                  >
